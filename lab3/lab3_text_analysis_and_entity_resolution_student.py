...
def countTokens(vendorRDD):
    ...
    """
    # return <FILL IN>
    return vendorRDD.map(lambda rec: len(rec[1])).reduce(lambda a, b : a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    # return <FILL IN>
    return vendorRDD.takeOrdered(10, lambda rec : -1 * len(rec[1]))
...
sims = similaritiesBroadcast.map(lambda (g,a,s): (a+' '+g ,s))
